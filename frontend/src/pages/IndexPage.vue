          ...
          <div class="">
            <q-card-section>
              <div class="row ">
                ...
                <q-btn dense unelevated class="drawer-toggle-btn" :icon="drawer ? 'chevron_left' : 'chevron_right'"
                  label="Upload files" color="primary" @click="drawer = !drawer"
                  :style="{ left: drawer ? '400px' : '0px', height: '36px', padding: '0 8px', marginLeft: '38px' }" />


                <!-- Left Drawer -->
                <q-drawer v-model="drawer" side="left" behavior="mobile" bordered :width="400">
                  <q-scroll-area class="fit q-pa-md">
                    <div class="text-h6 q-mb-md">Upload Files</div>

                    <q-file ref="fileInput" v-model="files" label="Upload " filled multiple   append  class="q-mb-md">

                      <template v-slot:selected>
                        <q-badge color="primary" v-if="files && files.length">
      {{ files.length }}
    </q-badge>
                      </template>
                    </q-file>
                    ...
                    <q-card v-if="files.length" class="q-mb-md">
                        <q-card-section>
                          ...
                          <!-- Header row: title on the left, two buttons on the right -->
                    ...
                    <q-btn color="primary" label="Process Data" class="full-width" icon="play_arrow" no-caps
                      :disable="files.length === 0" @click="uploadAndProcessFiles" />
                  </q-scroll-area>
                </q-drawer>

                <!-- Right Section: Filters & Table -->
                <div class="col-md-12 col-xs-12">
                  <div v-if="showFilters" class="row q-mb-md q-mt-md">

                    <!-- Date & Time Filters -->
                    <!-- Date & Time Filters (Combined Date-Time) -->
                    <q-input v-model="fromDate" label="From Date" type="date" dense filled
                      class="col-md-3 col-xs-12 q-pa-xs" />
                    <!-- <q-input v-model="fromTime" label="From Time" type="time" dense filled
                      class="col-md-3 col-xs-12 q-pa-xs" /> -->
                     <!-- From Time Picker with Seconds -->
<q-input
  v-model="fromTime"
  label="From Time"
  filled
        ...
        >
          <div class="row items-center justify-end">
            <q-btn v-close-popup label="Close" color="primary" flat />
                    ...
                    <q-input v-model="toDate" label="To Date" type="date" dense filled
                      class="col-md-3 col-xs-12 q-pa-xs" />
                    <!-- <q-input v-model="toTime" label="To Time" type="time" dense filled
                      class="col-md-3 col-xs-12 q-pa-xs" /> -->
<!-- To Time Picker with Seconds -->
<q-input
  v-model="toTime"
  label="To Time"
  filled
        ...
        >
          <div class="row items-center justify-end">
            <q-btn v-close-popup label="Close" color="primary" flat />
                    ...
                    <q-select v-model="selectedFilterCategory" :options="filterCategories" label="Filter By" dense
                      filled class="col-md-3 col-xs-12 q-pa-xs" emit-value map-options clearable
                      menu-anchor="bottom left" menu-self="top left" behavior="menu"
                      @update:model-value="updateFilterValues" />


                    <!-- Dynamic Value Dropdown/Input -->
                    <q-select v-if="isDropdownFilter" v-model="selectedFilterValue" :options="dynamicFilterOptions"
                      label="Select Value" dense filled class="col-md-3 col-xs-12 q-pa-xs" emit-value map-options
                      clearable use-input input-debounce="300" @filter="filterDropdownOptions" />


                    <q-input v-else v-model="selectedFilterValue" label="Enter Value" dense filled
                      class="col-md-3 col-xs-12 q-pa-xs" />

                    <!-- Transaction Type Filter -->
                    <!-- <q-select v-model="transactionTypeFilter" :options="transactionTypes" label="Transaction Type" dense
                      filled clearable class="col-md-3 col-xs-12 q-pa-xs" emit-value map-options /> -->
                      <q-select
                    ...
                    <!-- Reset Button -->
                    <q-btn color="primary" label="Reset" class="q-ml-md" dense
                      style="height: 38px; padding: 0 10px; margin-top: 4px;" @click="resetFilters" />
                  ...
                  :class="rows.length > 0 ? 'my-sticky-header-table' : ''"
                  :selection="rows.length > 0 ? 'multiple' : 'none'"
                  v-model:pagination="pagination"
                  :rows-per-page-options="[5, 9, 15, 20]"
                  :loading="loading"
                  :filter="search"
                  v-model:selected="selected"
                   ...
                   <div class="q-mt-md">
                    ...
                    <template v-slot:body-cell-amount="props">
                      <q-td :props="props">
                        <span :class="{
                          'text-green': props.row.transaction_type === 'Deposit',
                    ...
                    <template v-slot:body-cell-transaction_type="props">
                      <q-td :props="props">
                        <q-chip :color="props.row.transaction_type === 'Deposit' ? 'green' :
                          props.row.transaction_type === 'Withdrawal' ? 'orange' : 'grey'" text-color="white"
                          size="sm" dense>
                    ...
                    <template v-slot:body-cell-response_code="props">
                      <q-td :props="props">
                        <q-badge :color="props.row.response_code === '000' ? 'green' :
...
export default {
  ...
  return columns.value.filter(col => {
    ...
    const columns = ref([
     ...
     { name: "notes_dispensed_count", label: "Notes Dispensed Count", field: "notes_dispensed_count", sortable: true },
      { name: "notes_dispensed_t1", label: "Notes Dispensed T1", field: "notes_dispensed_t1", sortable: true },
      { name: "notes_dispensed_t2", label: "Notes Dispensed T2", field: "notes_dispensed_t2", sortable: true },
      { name: "notes_dispensed_t3", label: "Notes Dispensed T3", field: "notes_dispensed_t3", sortable: true },
      { name: "notes_dispensed_t4", label: "Notes Dispensed T4", field: "notes_dispensed_t4", sortable: true },
  ...
  try {
    ...
    if (response.status === 200 && response.data.transactions) {
      ...
      $q.notify({
        color: 'warning',
        message: response.data.error || 'No transactions found in the files',
        icon: 'info'
      });
      rows.value = [];
    }
    ...
    if (error.response && error.response.status === 401) {
      ...
      $q.notify({
        color: 'negative',
        message: error.response.data.message || "Trial period expired.",
        caption: error.response.data.error,
        icon: 'block'
      });
    } else {
      $q.notify({
...
//     };
    ...
    const addMoreFiles = () => {
  // will open the native file‑picker
      fileInput.value?.pickFiles();
    };

    // funtion to toggle row
    const toggleRow = (evt, row) => {
      const index = selected.value.findIndex(r => r.transaction_id === row.transaction_id)
      if (index > -1) {
        selected.value.splice(index, 1) // already selected → deselect
      } else {
        selected.value.push(row) // not selected → add
      }
    }
    ...
    const filteredRows = computed(() => {
      return rows.value.filter(row => {
        ...
        const matchesCategory = selectedFilterCategory.value && selectedFilterValue.value !== null
        ? row[selectedFilterCategory.value]?.toString() === selectedFilterValue.value?.toString()
        : true;
        ...
        //   : true;
        const matchesTransactionType = transactionTypeFilter.value && transactionTypeFilter.value.length > 0
          ? transactionTypeFilter.value.includes(row.transaction_type)
          : true;

        // Apply General Search Filtering (Search in All Columns)
        const matchesSearch = search.value
          ? Object.values(row).some(value =>
              value?.toString().toLowerCase().trim() === search.value.toLowerCase().trim()
            )
          : true;
